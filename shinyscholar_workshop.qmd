...
::: footer
[https://disagapp.le.ac.uk/](https://disagapp.le.ac.uk/)
:::
...
## Shiny modules {.mediumcode .mediumtext}

- Splitting code into self-contained chunks makes it easier to manage
...
- When used inside a module called `load`, `ns()` appends `load-` to the ID so `load-plot` and `load-number` are created
...
## Each module has several files associated with it {.smalltext}
...
## Module UI {.mediumcode}

- Just as you would develop a normal shiny UI apart from:
- Input IDs need wrapping in `ns()`
- Only inputs are included 
...
## The server contains an `observeEvent()` triggered by `input$run` {.mediumtext}
...
## The server contains an `observeEvent()` triggered by `input$run` {.mediumcode}
...
## The normal logger can't be accessed from inside async tasks {.mediumcode}
...
## Listen for messages in the server and then pass to the logger {.mediumcode}
```{r  eval = FALSE, echo = TRUE}
...
- When the app is saved, `common` is saved to and `.Rds` 
...
## `save_and_load()` takes care of tedious coding
...
## You can also just add data directly to the markdown
